    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#downLoad").click(function(){
                ...
                $("#downLoad").exprotAjaxExcel("/orderSelectAPI/orderOntimeExcel",paramData);
            });
        ...
        function getSelectDate(createDate,field){

            $("#selectTable").datagrid({
                url: "/orderSelectAPI/orderOntimeSelect?create_date="+createDate+"&&field="+field+"&&sendType="+sendType+"&&orderStatus="+orderStatus,
                ...
                columns: [[
                    {field:'orderSn',title:'订单编号',width: '16%', align: 'center',
                        formatter: function (val,row) {
                            return "流水号:"+row.orderNo+"</br>编号:"+row.orderId+"</br>下单日期:"+createDate;
                        }
                    },
        ...
        function getdata() {
            ...
            var startday = $("#startday").val();
            var endday = $("#endday").val();
            var paramData = {'startday': startday, 'endday': endday,'datatype':datatype,'orderStatus':orderStatus};
            $.postJSON("/orderSelectAPI/orderOntimeCount", paramData, function (data) {
                $('#tableList').datagrid({
                    iconCls: 'icon-save',
                    fitColumns:false,
                    rownumbers: true,//行号
                    ...
                    pagination:false,
                    frozenColumns:[
                        [{"field":"create_date","title":"时间","width":180}]
                    ],
                    columns: JSON.parse(data.obj.titleStr),
                    onDblClickCell :function(index, field){
                        createDate = $("#tableList").datagrid("getData").rows[index].create_date;
                        selectField = field;
                        if(createDate=="合计")return;
                        var str = field.substring(0,1);
                        if("d"==str)str="点我达";
                        else if("m"==str)str="美团";
                        else if("e"==str)str="饿了么";
                        if(orderStatus==2)str = str+ "---预送订单";
                        if(orderStatus==1)str = str+ "---常规订单";
                        $('#dd').window('open');
                        $('#dd').dialog({
                            iconCls:'icon-save',
                            title:"日期:"+createDate+"   配送方式:"+str,
                            resizable:true,
                            maximizable:true,
                            collapsible:true,
                            minimizable:true,
                        ...
                        $("#cc").change(function(){
                            sendType = $("#cc").val();
                            getSelectDate(createDate,selectField);
                        });
                        getSelectDate(createDate,field);
                    }
    ...
    <div class="inset_auto">
        <div class="main">
            ...
            <div class="cation" style="float: right;display: none;" id="datatype">
                <a href="" class="nowcation" datatype="1">点我达</a>
                <a href="" datatype="2">美团</a>
                <a href="" datatype="3">饿了么</a>
            </div>
