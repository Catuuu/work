            ...
            $('#tb1').datagrid({
                ...
                columns: [[
...

                    {
                        ...
                        var str = '<div style="margin: 5px;"> <div><a id="orderstatusview' + row.rownumber + '"  onclick="orderstatusview(\'' + row.order_id + '\',' + row.rownumber + ',\'' + row.create_date + '\')" href="javascript:void(0)">';
...
//                        str += '</a></div>';
        ...
        //查看订单状态
        function orderstatusview(order_id, rownumber, create_date) {
            $.ajax({
                url: "/orderSelectAPI/viewhistoryorderlog",
                type: 'POST',
                data: {order_id: order_id, create_date: create_date},
                success: function (result) {
                    if (result.status == "1") {
                        if (result.obj != null) {
                            var content = "";
...
//                $("#class_id").get(0).options[0].selected = true;
        ...
        function delGiftInfo(index, n) {
            ...
            $('#giftInfoSelect').datagrid('unselectRow', n - 1);
    ...
    <div id="selectForm" name="selectForm">
        ...
        <div class="easyui-panel" style="padding:5px;background:#fff;height:40px;">
            ...
            <input class="easyui-textbox" name="fromin_no" style="width:180px"
                   data-options="label:'来源流水号：',prompt:'来源流水号'">
