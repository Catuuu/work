...
//                $(this).addClass("brandselect").siblings().removeClass("brandselect");
        ...
        function getdata(brand_id) {
            ...
            if(typeof(brand_id) == "undefined")brand_id=0;
            var paramData = {
                'startday': startday,
                'endday': endday,
                'ordertype': ordertype,
            ...
            $.postJSON("/orderSelectAPI/order_count", paramData, function (data) {
                var brandStr = " <b>品牌：</b>";
                for(let i in data.obj.brand){
                    brandStr += "<a  onclick=\"getdata("+data.obj.brand[i].brand_id+")\" data-status=\""+data.obj.brand[i].brand_id+"\" class=\"easyui-linkbutton  brandselect\"\n" +
                        "data-options=\"toggle:true,group:'g4',plain:true\">&nbsp;&nbsp;&nbsp"+data.obj.brand[i].brand_name+"&nbsp;&nbsp;</a>"
                }
                $(".selectbrand").html(brandStr);
                $("#goodList").empty();
                for (let i in data.obj.list) {
                    ...
                    var goodtd = '<tr class="table_content">' +
                        '<td>' + topindex + '</td>' +
                        '<td>' + data.obj.list[i].timeDay + '</td>' +
                        '<td>' + data.obj.list[i].sum1 + '</td>' +
                        '<td>' +(data.obj.list[i].sum3/data.obj.list[i].sum1).toFixed(2).replace("NaN","0") + '</td>' +
                        '<td>' + data.obj.list[i].sum3 + '</td>' +

                        '<td>' + data.obj.list[i].elm1 + '</td>' +
                        '<td>' + (data.obj.list[i].elm3/ data.obj.list[i].elm1).toFixed(2).replace("NaN","0") + '</td>' +
                        '<td>' + data.obj.list[i].elm3 + '</td>' +

                        '<td>' + data.obj.list[i].mt1  + '</td>' +
                        '<td>' + (data.obj.list[i].mt3/data.obj.list[i].mt1).toFixed(2).replace("NaN","0") + '</td>' +
                        '<td>' + data.obj.list[i].mt3 + '</td>' +

                        '<td>' + data.obj.list[i].bdwm1 + '</td>' +
                        '<td>' + (data.obj.list[i].bdwm3/ data.obj.list[i].bdwm1).toFixed(2).replace("NaN","0") + '</td>' +
                        '<td>' + data.obj.list[i].bdwm3 + '</td>' +

                        '<td>' + data.obj.list[i].wx1 + '</td>' +
                        '<td>' + (data.obj.list[i].wx3/ data.obj.list[i].wx1).toFixed(2).replace("NaN","0") + '</td>' +
                        '<td>' + data.obj.list[i].wx3 + '</td>' +
                        '</tr>';
                ...
                $('#container_line').highcharts({
                    ...
                    title: {
                        text: '<b>订单管理-' + data.obj.reporttitle + '</b>',
                        x: -20 //center
                    },
                    subtitle: {
    ...
    <div class="inset_auto">
        <div class="main">
            ...
            <div class="cation" style="float: right;display: none;" id="datatype">
                <a href="" class="nowcation" datatype="1">订单数</a>
                <a href="" datatype="2">客单价</a>
                <a href="" datatype="3">销售额</a>
            </div>
