...
/**
 * Created by Administrator on 2017/2/4. 点我达消息处理
 */
@Controller
@RequestMapping("/DianWoDan")
public class DianWoDanController extends BasicController {
    @Resource(name = "WeiXinService")
    protected WeiXinService weiXinService;

    @ResponseBody
    @RequestMapping("OrderStatus")
    @ResourceMethod(name = "接收点我达推送消息", check = NO_CHECK)
    public String OrderStatus(){

        DwdMessageParam message =  BeanUtil.createBean(WebUtil.getRequest(),DwdMessageParam.class);

        jmsTemplate.send("dianWoDan.queue", new MessageCreator() {
            @Override
    ...
    @ResponseBody
    @RequestMapping("OrderStatus_bak")
    @ResourceMethod(name = "接收点我达推送消息", check = NO_CHECK)
    public String OrderStatus_bak(){
        // Map params =  BeanUtil.createBean(WebUtil.getRequest(),HashMap.class);
        // System.out.println("接收订单状态:"+ JSONObject.toJSONString(params));
        DwdMessageParam message =  BeanUtil.createBean(WebUtil.getRequest(),DwdMessageParam.class);
